...
Fruitendo for iOS can be run directly on your device without the need for jailbreaking. To do this, you will need a few things:

* Your own iOS Apple developer account
* Your developer account set up on your computer (your certs, etc.)
* A Distribution provisioning profile for Fruitendo (a wildcard profile is fine and suggested)

Once you have all of this stuff, getting Fruitendo on a non-jailbroken device is pretty simple.

## Config

Fruitendo needs to know a couple things when building the app. You can configure these under `apple/script/build.config` inside the Fruitendo repo. Once you initially clone down the repo, go into this file and make the changes.
...
When Fruitendo is built into an IPA, it will embed this provisioning profile into the IPA so you can just install the IPA right onto your phone.


## Building

After you've configured the right things, you're ready for building. You'll want to run these from the `apple` directory of the Fruitendo project.
...
This will clone down their repos, build them into the `libretro-super/dist/ios` directory. Once this is complete, you need to copy them into the appropriate directory for Fruitendo iOS. The path is:

`Fruitendo/apple/iOS/modules`

### Build Fruitendo iOS app
...
Once completed, you can find the IPA inside the `apple/build/Release-iphoneos` directory. Excecute the IPA file and then open iTunes, with you device connected, click in your device, then Applications, find de Fruitendo in the list, click install and syncronize with you device.

## Roms

iTunes File Sharing is enabled on Fruitendo. You can simply drag your rom files into the Fruitendo app. They will be available the next time you launch the app.
